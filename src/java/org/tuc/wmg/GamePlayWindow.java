/*
 * GamePlayWindow - This file is part of Whack-Mole-Game
...
/**
 * This is the new window to show the game after starting a new game. It consists of 3 parts
 * including the information and current results of the game, a visualization for game status and a
 * panel of the control buttons (the buttons are unused for now).
 * 
 * @author Chenfeng Zhu
 *
 */
public class GamePlayWindow extends JPanel {
    ...
    public void updateInfoLabel() {
        ...
        sb.append("Result: "
                + (1.0 * server.getCurrentGame().getHitTimes() / server.getCurrentGame().getCurrentTimes())
                + "<br/>");
    ...
    public void refreshPlayPanel(int mid, MoleStatus status) {
        for (int i = 0; i < listLabelMoles.size(); i++) {
            ...
            if (mote.getMid() == mid) {
                ...
                case HIT:
                    label.setText("+++ " + i + mote + " +++");
                    imgURL = GamePlayWindow.class.getClassLoader().getResource(
                ...
                java.net.URL imgURL = GamePlayWindow.class.getClassLoader().getResource(
                        "resources/images/circle_red_48px.png");
                ImageIcon iconRed = new ImageIcon(imgURL);
                label.setIcon(iconRed);
            }
